...
async function mostrarProductos() {
  try {
    const resultado = await conexion.query("SELECT * FROM producto ORDER BY descripcion", {
...
async function crearProducto(descripcion, precio) {
  try {
    const resultado = await conexion.query("INSERT INTO producto VALUES (NULL, ?, ?)", {
...
async function actualizarProducto(id, nuevoPrecio) {
  try {
    const resultado = await conexion.query("UPDATE producto SET precio = :nuevoPrecio WHERE id = :id", {
...
async function buscarUsuario(nombre, contrasena) {
  try {
    const resultado = await conexion.query("SELECT * FROM usuario WHERE usuario = :nombre AND contrasena = :contrasena", {
      type: sequelize.QueryTypes.SELECT,
...
server.get("/datosUsuario", esUnTokenValido, (req, res, err) => {
  const tokenDelRequest = jwt.verify(req.headers.authorization.split(" ")[1], CLAVE_CIFRADO_SERVER);
...
async function crearUsuario(usuario, contrasena, nombre_apellido, e_mail, telefono, direccion) {
  try {
    const resultado = await conexion.query("INSERT INTO usuario VALUES (NULL, ?, ?, ?, ?, ?, ?, 'F')", {
...
function esUnTokenValido(req, res, next) {
  try {
    const usuario = jwt.verify(req.headers.authorization.split(" ")[1], CLAVE_CIFRADO_SERVER);
...
async function crearPedido(forma_de_pago, id_usuario) {
  try {
    var fecha = new Date();
    const resultado = await conexion.query("INSERT INTO pedido VALUES (NULL, 'NUE', ?, ?, ?)", {
...
async function crearDetallePedido(id_pedido, id_producto, cantidad_platos) {
  try {
    const resultado = await conexion.query("INSERT INTO detalle_pedido VALUES (NULL, ?, ?, ?)", {
...
async function actualizarEstadoPedido(id, nuevoEstado) {
  try {
    const resultado = await conexion.query("UPDATE pedido SET estado = :nuevoEstado WHERE id = :id", {
...
async function borrarDetallePedido(id_pedido) {
  try {
    const resultado = await conexion.query("DELETE FROM detalle_pedido WHERE id_pedido = :id_pedido", {
...
async function borrarPedido(id) {
  try {
    const resultado = await conexion.query("DELETE FROM pedido WHERE id = :id", {
      replacements: { id: id },
    });
    console.log(resultado);
  } catch (e) {
